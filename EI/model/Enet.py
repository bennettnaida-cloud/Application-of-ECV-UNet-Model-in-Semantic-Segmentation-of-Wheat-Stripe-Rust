...
class ENet(nn.Module):
    """修改后的ENet模型，接口适配 in_ch 和 out_ch

    Keyword arguments:
    - in_ch (int): 输入通道数，默认为3
    - out_ch (int): 输出通道数（即分类数），默认为1
    """

    def __init__(self, in_ch=3, out_ch=1):
        super().__init__()

        # 设置encoder和decoder的激活函数默认值
        encoder_relu = False  # 原论文中encoder使用PReLU
        decoder_relu = True  # 原论文中decoder使用ReLU

        # 修改输入通道
        self.initial_block = InitialBlock(in_ch, 16, relu=encoder_relu)

        # Stage 1 - Encoder （保持原结构）
        ...
        # Stage 2 - Encoder （保持原结构）
        self.downsample2_0 = DownsamplingBottleneck(
            64, 128, return_indices=True, dropout_prob=0.1, relu=encoder_relu)
        self.regular2_1 = RegularBottleneck(128, padding=1, dropout_prob=0.1, relu=encoder_relu)
        self.dilated2_2 = RegularBottleneck(128, dilation=2, padding=2, dropout_prob=0.1, relu=encoder_relu)
        self.asymmetric2_3 = RegularBottleneck(
            128, kernel_size=5, padding=2, asymmetric=True, dropout_prob=0.1, relu=encoder_relu)
        self.dilated2_4 = RegularBottleneck(128, dilation=4, padding=4, dropout_prob=0.1, relu=encoder_relu)
        self.regular2_5 = RegularBottleneck(128, padding=1, dropout_prob=0.1, relu=encoder_relu)
        self.dilated2_6 = RegularBottleneck(128, dilation=8, padding=8, dropout_prob=0.1, relu=encoder_relu)
        self.asymmetric2_7 = RegularBottleneck(
            128, kernel_size=5, asymmetric=True, padding=2, dropout_prob=0.1, relu=encoder_relu)
        self.dilated2_8 = RegularBottleneck(128, dilation=16, padding=16, dropout_prob=0.1, relu=encoder_relu)

        # Stage 3 - Encoder （保持原结构）
