...
class CBAM_ECAUNet(nn.Module):
    """集成ViT的改进版U-Net"""

    def __init__(self, in_ch=3, out_ch=1):
        super(CBAM_ECAUNet, self).__init__()
    ...
    def forward(self, x):
        ...
        cbam_e1=self.cbam_e1(e1)
        e2 = self.Maxpool(cbam_e1)
        e2 = self.Conv2(e2)
        eca_e2 = self.eca_e2(e2)
        cbam_e2 = self.cbam_e2(e2)

        e3 = self.Maxpool(cbam_e2)
        e3 = self.Conv3(e3)
        eca_e3 = self.eca_e3(e3)
        cbam_e3 = self.cbam_e3(e3)
        e4 = self.Maxpool(cbam_e3)
        e4 = self.Conv4(e4)
        eca_e4 = self.eca_e4(e4)
        cbam_e4 = self.cbam_e4(e4)
        # ViT编码层 [1](@ref)
        e5 = self.Maxpool(cbam_e4)
        e5 = self.Conv5(e5)  # 输出形状保持[bs,1024,8,8]
