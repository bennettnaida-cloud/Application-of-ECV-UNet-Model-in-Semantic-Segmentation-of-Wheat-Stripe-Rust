...
def input_images(x, y, i, n_iter, k, run_dir):  # 新增 run_dir 参数
    """
    ...
    if k == 1:
        ...
        ax1.set_xticklabels([])
        ax1.set_yticklabels([])
        save_path = os.path.join(os.path.join(run_dir, 'L'),
                                 str(n_iter - 1) + '_epoch_'
                                 + str(i))
...
def plot_kernels(tensor, n_iter, num_cols, run_dir, cmap="gray"):
    ...
    for t1 in t:
        ...
        if b == 20:
            break
    save_path = os.path.join(os.path.join(run_dir, 'Kernal'),
                             str(n_iter - 1) + '_epoch_'
        + str(i))
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path)
